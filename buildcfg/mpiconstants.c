...
int main(int argc,char ** args)
{
  ...
  fputs("\n",file);
  fputs("  -- Data Types\n",file);
  fprintf(file,"  MPI_BYTE : constant := 16#%x#;\n",MPI_BYTE);
  fputs("\n",file);
  fputs("  -- Errors\n",file);
  fputs("\n",file);
  fprintf(file,"  MPI_SUCCESS       : constant := 16#%x#;\n",MPI_SUCCESS);
  fprintf(file,"  MPI_ERR_BUFFER    : constant := 16#%x#;\n",MPI_ERR_BUFFER);
  fprintf(file,"  MPI_ERR_COUNT     : constant := 16#%x#;\n",MPI_ERR_COUNT);
  fprintf(file,"  MPI_ERR_TYPE      : constant := 16#%x#;\n",MPI_ERR_TYPE);
  fprintf(file,"  MPI_ERR_TAG       : constant := 16#%x#;\n",MPI_ERR_TAG);
  fprintf(file,"  MPI_ERR_COMM      : constant := 16#%x#;\n",MPI_ERR_COMM);
  fprintf(file,"  MPI_ERR_RANK      : constant := 16#%x#;\n",MPI_ERR_RANK);
  fprintf(file,"  MPI_ERR_REQUEST   : constant := 16#%x#;\n",MPI_ERR_REQUEST);
  fprintf(file,"  MPI_ERR_ROOT      : constant := 16#%x#;\n",MPI_ERR_ROOT);
  fprintf(file,"  MPI_ERR_GROUP     : constant := 16#%x#;\n",MPI_ERR_GROUP);
  fprintf(file,"  MPI_ERR_OP        : constant := 16#%x#;\n",MPI_ERR_OP);
  fprintf(file,"  MPI_ERR_TOPOLOGY  : constant := 16#%x#;\n",MPI_ERR_TOPOLOGY);
  fprintf(file,"  MPI_ERR_DIMS      : constant := 16#%x#;\n",MPI_ERR_DIMS);
  fprintf(file,"  MPI_ERR_ARG       : constant := 16#%x#;\n",MPI_ERR_ARG);
  fprintf(file,"  MPI_ERR_UNKNOWN   : constant := 16#%x#;\n",MPI_ERR_UNKNOWN);
  fprintf(file,"  MPI_ERR_TRUNCATE  : constant := 16#%x#;\n",MPI_ERR_TRUNCATE);
  fprintf(file,"  MPI_ERR_OTHER     : constant := 16#%x#;\n",MPI_ERR_OTHER);
  fprintf(file,"  MPI_ERR_INTERN    : constant := 16#%x#;\n",MPI_ERR_INTERN);
  fprintf(file,"  MPI_ERR_IN_STATUS : constant := 16#%x#;\n",MPI_ERR_IN_STATUS);
  fprintf(file,"  MPI_ERR_LASTCODE  : constant := 16#%x#;\n",MPI_ERR_LASTCODE);
  fputs("  -- Other...\n",file);
  fprintf(file,"  MPI_ANY_SOURCE : constant:=16#%x#;\n",MPI_ANY_SOURCE);
  fprintf(file,"  MPI_ANY_TAG    : constant:=16#%x#;\n",MPI_ANY_TAG);
  fputs("\n",file);
